...
from domain.predict_repeater import PredictRepeater
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print("Please input repeat times")
        ...
        for i, data in enumerate(dataset):
            ...
            if date_list[-1].date() == date.date():
                # check repeat times
                if i + repeat_times - 1 < len(dataset):
                    test_index = i
                break
                print("Found the date")

        if test_index == -1:
            print("The date is not in the dataset or repeat times is too large")
            continue
    ...
    dataset.set_return_with_date(False)
    for repeat in range(repeat_times):
        _, y = dataset[test_index + repeat]
        total_y.append(y)
    ...
    for repeat in range(repeat_times):
        ...
        # predict
        predict_repeater = PredictRepeater(model)
        output = model(x)
        ...
        total_output.append(output[0][-1])

        # loss
        loss = criterion(output[:, -1, :], total_y[repeat])
        print(f"RMSE Loss for {repeat + 1}th prediction: {loss.item()}")
    ...
    # change to tensor
    total_y = torch.stack(total_y).to(device)
    total_output = torch.stack(total_output).to(device)
